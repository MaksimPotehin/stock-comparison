  ...
  <div ref="containerRef" class="relative">
    ...
    <div
      ...
      class="absolute z-50 top-[calc(100%+4px)] left-0 right-0
       bg-card rounded-lg border border-sb-border overflow-hidden"
      style="box-shadow: var(--shadow-dropdown)"
    >
      <!-- Loading -->
      <div v-if="isLoading" class="px-4 py-3 text-sm text-sb-muted flex items-center gap-x-2">
      ...
      <!-- Error -->
      <div v-else-if="errorMsg" class="px-4 py-3 text-sm text-danger">
        {{ errorMsg }}
      </div>
      ...
      <template v-else>
        <button
          ...
          :key="item.symbol"
          class="w-full flex items-center justify-between px-4 py-2.5 hover:bg-sb-hover transition-colors text-left"
          @click="select(item)"
        >
          <div class="flex items-center gap-x-3 min-w-0">
            <span class="font-mono font-semibold text-sb-text text-sm shrink-0">{{ item.symbol }}</span>
            <span class="text-sb-secondary text-sm truncate">{{ item.name }}</span>
          </div>
          <span class="text-sb-subtle text-xs shrink-0 ml-2">{{ item.exchange }}</span>
        </button>
      </template>
    </div>
  </div>
