...
function calculateSummary ({ columns, data }: { columns: any[]; data: T[] }): string[] {
  const summary = columns.map((column: any) => {
    if (column.property === 'initialDeposit') {
      ...
      return lastRow ? (parseFloat(lastRow.initialDeposit) || 0).toFixed(2) : '0.00'
    } else if (column.property === 'totalAmount') {
      const lastRow = data[data.length - 1]
      if (lastRow) {
        return parseFloat(lastRow.totalAmount).toFixed(2)
      }
      const totalAccValue = data.reduce((sum, row: T) => sum + parseFloat(row.initialDeposit || 0), 0)
      const totalInterest = data.reduce((sum, row: T) => sum + parseFloat(row.interestPerPeriod || 0), 0)
      return (totalAccValue + totalInterest).toFixed(2)
    } else if (column.property === 'interestPerPeriod') {
      return data.reduce((sum, row: T) => sum + parseFloat(row.interestPerPeriod || 0), 0).toFixed(2)
    }
    return ''
  })
  return summary
